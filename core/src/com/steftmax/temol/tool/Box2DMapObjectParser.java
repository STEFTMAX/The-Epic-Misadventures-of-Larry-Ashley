 ...
 * Just create a new {@link Box2DMapObjectParser2} in any way you like and call {@link #load(World, MapLayer)} to load all compatible objects (defined by the {@link Aliases}) into your {@link World}.<br/>
 * <br/>
...
public class Box2DMapObjectParser {
	...
	/** creates a new {@link Box2DMapObjectParser2} with the default {@link Aliases} */
	public Box2DMapObjectParser() {
		this(new Aliases());
	}

	/**
	 * creates a new {@link Box2DMapObjectParser2} using the given {@link Aliases}
	...
	/**
	 * creates a new {@link Box2DMapObjectParser2} using the given {@link #unitScale unitScale} and sets {@link #ignoreMapUnitScale} to true
	...
	/**
	 * creates a new {@link Box2DMapObjectParser2} using the given {@link #unitScale}, {@link #tileWidth}, {@link #tileHeight} and sets {@link #ignoreMapUnitScale} to true
	...
	/**
	 * creates a new {@link Box2DMapObjectParser2} using the given {@link Aliases} and {@link #unitScale} and sets {@link #ignoreMapUnitScale} to true
	 * @param aliases the {@link Aliases} to use
	 * @param unitScale the {@link #unitScale} to use
	 */
	...
	/**
	 * creates a new {@link Box2DMapObjectParser2} with the given parameters and sets {@link #ignoreMapUnitScale} to true
	 * @param aliases the {@link Aliases} to use
	 * @param unitScale the {@link #unitScale unitScale} to use
	 * @param tileWidth the {@link #tileWidth} to use
	 * @param tileHeight the {@link tileHeight} to use
	...
	public World load(World world, MapLayer layer) {
		for(MapObject object : layer.getObjects()) {
			...
			if(type == null || type == "") {
				Gdx.app.debug(getClass().getName(), "Type on object is null, skipping");
				continue;
			}
	...
	public Body createBody(World world, MapObject mapObject) {
		...
		bodyDef.position.set((Float)getProperty(properties, "x", bodyDef.position.x, Integer.class) * unitScale, (Float) getProperty(properties, "y", bodyDef.position.y, Float.class) * unitScale);
	...
	public Fixture createFixture(MapObject mapObject) {
		...
		if(mapObject instanceof RectangleMapObject) {
			...
			((CircleShape) shape).setPosition(new Vector2(circle.x + circle.radius, circle.y+ circle.radius));
